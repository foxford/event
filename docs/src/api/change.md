...
event_type          | string        | _optional_ | New value for the event type.
event_set           | string        | _optional_ | New value for the event set.
event_label         | string        | _optional_ | New value for the event label.
event_data          | json          | _optional_ | New value for the event data.
event_occurred_at   | int           | _optional_ | New value for the event occurred_at, timestamp in nanoseconds.
